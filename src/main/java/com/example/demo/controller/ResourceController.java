...
public class ResourceController {
	...
	public HashMap<String, String> getUser(@PathVariable int id) {
		...
		user.put("country", users.get().getCountry());
		user.put("image",users.get().getImage());
		
		return user;
	}
	
	@CrossOrigin(origins="http://localhost:3000")
	@GetMapping("/jpa/get-institute/{id}")
	public HashMap<String, String> getInstitute(@PathVariable int id) {
	...
	@CrossOrigin(origins="http://localhost:3000")
	@PutMapping("/jpa/update-info/{id}")
	public String updateUser(@RequestBody User user,@PathVariable int id) {

		System.out.println(user);
		repository.updateUser(id,user.getDob(),user.getFirstName(),user.getLastName(),user.getCountry(),user.getImage(),user.getPhone());
		return "Details updated";
	}
	
	@CrossOrigin(origins="http://localhost:3000")
	@PutMapping("/jpa/update-institute/{id}")
	public String updateInstitute(@RequestBody User user,@PathVariable int id) {

	
		repository.updateInstitute(id,user.getGrade(),user.getInstitute());
		return "Details updated";
	}
	...
	public int createQuiz(@RequestBody Quiz quiz,@PathVariable int id) {
		
		Optional<Topic> topic =topicRepository.findById(id);
		quiz.setTopics(topic.get());
		quiz.setTimestamp(new Timestamp(System.currentTimeMillis()));
		quizRepository.save(quiz);
		List<Quiz> quizs=topic.get().getQuiz();
		int len=quizs.size();
		return quizs.get(len-1).getId();
	}
	...
	@CrossOrigin(origins="http://localhost:3000")
	@PutMapping("/jpa/{id}/edit-quiz")
	public void editQuiz(@RequestBody Quiz quiz,@PathVariable int id) {
		
			
		Optional<Quiz> quizs=quizRepository.findById(id);
		
		if(quiz.getMarksScored()!=0) {
			quizs.get().setMarksScored(quiz.getMarksScored());
		}
		
		if(quiz.getCorrectAnswers()!=null) {
			quizs.get().setCorrectAnswers(quiz.getCorrectAnswers());
		}
		
		if(quiz.getSelectedAnswers()!=null) {
			quizs.get().setSelectedAnswers(quiz.getSelectedAnswers());
		}
		
		quizRepository.save(quizs.get());
...
;	}
	...
	@CrossOrigin(origins="http://localhost:3000")
	@PutMapping("/jpa/{id}/change-password")
	public String change_password(@RequestBody User user, @PathVariable int id) {
	...
	@CrossOrigin(origins="http://localhost:3000")
	@GetMapping("/jpa/{id}/get-email")
	public String get_email(@PathVariable int id) {
		Optional<User> user=repository.findById(id);
